...
public class CallsLoggingAspect {
  ...
  private String prepareAfterThrowing(ProceedingJoinPoint joinPoint, Throwable t) {
    return "Execution of method "
        + getCallingMethod(joinPoint) + " failed with exception " + t.getMessage();
  }
  ...
  String matchNamesWithArgs(String[] paramNames, Object[] args) {
    if (paramNames == null || paramNames.length == 0) {
      ...
      throw new IllegalStateException("Param names count " + paramNames.length
          + " don't matches actual args count " + args.length);
    } else {
