...
public class ExcelExport {
  ...
  public void addDivider(
      Workbook workbook, String sheetName, String divider, Integer rowNumber, Integer cellsToMerge
  ) {
  ...
  public void addHeader(
      Workbook workbook, String sheetName, Integer rowNumber, List<ExportDefiner> definers
  ) {
    Sheet sheet = workbook.getSheet(sheetName);
    // header
    if (definers != null) {
   ...
   */
  public <T> Integer addContent(
      Workbook workbook, String sheetName, Integer rowNumber,
      List<ExportDefiner> definers, List<T> entities
  ) throws ReflectiveOperationException {
    Sheet sheet = workbook.getSheet(sheetName);
   ...
   * @throws IllegalAccessException
   */
  private <T> int prepareContent(
      Row row, CellStyle style, int index, List<ExportDefiner> definers, T entity
  ) throws ReflectiveOperationException {
    for (ExportDefiner definer : definers) {
      if (entity == null) {
   ...
   * @throws ReflectiveOperationException
   */
  private <T> Object recognize(
      ExportDefiner definer, T entity
  ) throws ReflectiveOperationException {
    Class<?> aClass = entity.getClass();
    List<Field> fields = Arrays.asList(aClass.getDeclaredFields());
    Optional<Field> possibleField = fields.stream()
        .filter(f -> f.getName().equals(definer.getName())).findFirst();
    if (possibleField.isPresent()) {
      ...
      try {
        if (definer.getParameters() == null) {
          ...
          return aClass.getDeclaredMethod(definer.getName(), definer.getClasses())
              .invoke(entity, definer.getParameters());
        }
